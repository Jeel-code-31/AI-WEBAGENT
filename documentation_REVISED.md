...
*   **Hero Section Identification**: Automatically detecting the primary "Hero Image" of a website when standard Social Metadata (OpenGraph) is missing.
...
### 3. Automated Hero Image Detection (Fallback)
*   **Problem**: What if the website has no `og:image` metadata?
*   **Solution**: We implemented a fallback logic that scans for the largest `<img>` tag or the first image within a `<header>` or `<section>` with a `hero` class/ID. This ensures the "Social Graph" preview always has a visual representative of the site.

### 4. No-Referrer Policy for Media
*   **Problem**: Secured sites (like Google) block images if the request comes from your domain.
*   **Solution**: We added `referrerPolicy="no-referrer"` to the `<img>` tags. This instructs the browser to hide where the request is coming from, effectively bypassing most hotlinking protections.

### 5. Advanced RegEx CSS Parsing
